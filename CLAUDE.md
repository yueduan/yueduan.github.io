...
**CSS note:** The site loads `css/resume.min.css`, which is the **live stylesheet**. Despite the name, it is not a minified copy of `resume.css` — the two files have diverged. `resume.min.css` contains the original template CSS (minified) on line 1, followed by newer custom styles appended in plain CSS (lines 2–284). When editing styles, edit `css/resume.min.css` directly (it is the authoritative file). `css/resume.css` contains only the original template styles and is not loaded by any page.
